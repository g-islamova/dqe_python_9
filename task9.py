...
Input format for JSON file to be processed:
{
  "type": "news",
  "text": "News text content goes here",
  "city": "CityName"
}
{
  "type": "private ad",
  "text": "Private ad text content goes here",
...
class JsonParser:
    def __init__(self, folder_path: str = None):
        if folder_path:
            ...
            self.folder_path = r"C:\Users\Guzel_Islamova\Documents\json_files"

    def parse_json(self, news_feed: NewsFeed) -> bool:
        try:
...
class XmlParser:
    def __init__(self, folder_path: str = None):
        ...
        if folder_path:
            ...
            self.folder_path = r"C:\Users\Guzel_Islamova\Documents\xml_files"

    def parse_xml(self, news_feed: NewsFeed) -> bool:
        try:
            if os.path.exists(self.folder_path):
                ...
                if not files:
                    ...
                    for file_name in files:
                        ...
                        for record in root.findall('record'):
                            record_type = record.find('type').text.strip().lower()
                            if record_type == "news":
                                news_feed.add_record(News(record.find('text').text, record.find('city').text))
                            elif record_type == "private ad":
                                expiration_date = datetime.strptime(record.find('expiration_date').text,
                                                                    "%d/%m/%Y")
                                news_feed.add_record(PrivateAd(record.find('text').text, expiration_date))
                            elif record_type == "weather":
                                news_feed.add_record(
                                    Weather(record.find('city').text, int(record.find('temperature').text)))
                    os.remove(os.path.join(self.folder_path, file_name))
                return True
            else:
                print("Folder not found at the specified path.")
...
def main():
    ...
    default_folder_path = r"C:\Users\Guzel_Islamova\Documents\json_files"
    news_feed = NewsFeed()
    ...
    while True:
        try:
            ...
            if choice == 1:
                ...
                success = json_parser.parse_json(news_feed)
                if success:
                    news_feed.save_to_file()
                ...
                success = xml_parser.parse_xml(news_feed)
                if success:
                    news_feed.save_to_file()
